...
export default function OrderDetailPage({ params }) {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      console.log('Order decoded:', decoded);
      console.log('Items image URLs:', decoded.items?.map((i) => i.image));
    }
    setOrder(decoded);
  }, [params.token]);
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-6">
        ...
        {store && (
          <section className="mt-3">
            <Card title="Outlet Pengambilan" icon={<Icon.Pin size={16} />}>
              <div className="flex items-start gap-3">
                <div className="w-12 h-12 rounded-2xl bg-ink-100 grid place-items-center shrink-0 text-ink-500">
                  <Icon.Store size={22} />
                </div>
              ...
              {items.map((it, i) => (
                <li key={i} className="py-3 first:pt-0 last:pb-0 flex gap-3">
                  <div className="w-20 h-20 rounded-xl bg-ink-100 overflow-hidden shrink-0">
                    ...
                    <div
                      className="w-full h-full grid place-items-center text-ink-400"
                      style={{ display: it.image ? 'none' : 'grid' }}
                    >
                      <Icon.Coffee size={28} />
                    </div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="bg-ink-900 text-white text-[10px] font-bold min-w-[22px] h-[22px] px-2 rounded-full grid place-items-center shrink-0">
                        {it.qty}×
                      </span>
                      <p className="font-semibold text-sm text-ink-900 leading-snug line-clamp-2">
                        {it.name}
                      </p>
                    </div>
                    {it.variant && (
        ...
        <section className="mt-3">
          <div className="rounded-2xl bg-white border border-ink-200 overflow-hidden">
            <div className="px-4 md:px-5 py-3.5 border-b border-ink-100">
              <div className="flex items-center gap-2">
                <span className="w-8 h-8 rounded-lg bg-ink-100 grid place-items-center text-ink-700">
